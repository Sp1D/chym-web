...
"@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$";
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (user.getP1().length() < passwordMin || user.getP1().length() > passwordMax) {
            errors.rejectValue("p1", "password.length", "Password must be " + passwordMin + " to " + passwordMax + " characters");
        }

        if (!user.getP1().equals(user.getP2())) {
            errors.rejectValue("p2", "Passwords are not match", "Passwords are not match");
        }
    }
